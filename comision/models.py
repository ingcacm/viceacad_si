...
class comisionv2(models.Model):
        ...
        facultad = models.CharField(max_length=2, choices=FACULTAD1)
        proy_curricular = models.CharField(max_length=100)
        CATEGORIA_DOC = (
                ('A', 'Asistente'),
                ('B', 'Asociado'),
                ('C', 'Auxiliar'),
	...
	regimen_docente = models.CharField(max_length=1, choices= REGIMEN_DOCENTE)
	TIP_ESTUDIO = (
                ('A', ' Maestria'),
                ('B', ' Doctorado'),
                ('C', ' Posdoctorado'),
	...
	regimen_comision = models.CharField(max_length=1, choices=REG)
        duracion_estudios_meses = models.IntegerField()
        ORIGEN_RES = (
                ('A', 'CSU'),
                ('B', 'CA'),
	...
	csu_ca_res = models.CharField(max_length=1, choices=ORIGEN_RES)
        fecha_res_CSU_CA = models.DateField()
        num_res_aprobacion_CSU = models.CharField(max_length=15)
        num_contrato = models.CharField(max_length=10)
        fecha_contrato = models.DateField()
	...
	otrosi = models.CharField(max_length=1, choices=OTRO_SI)
        fecha_otrosi = models.DateField(null=True, blank=True)
        obj_otrosi = models.TextField(max_length=2000, blank=True)
        valor_otrosi = models.IntegerField(null=True, blank=True)
        salario_RRHH_otrosi = models.IntegerField(null=True, blank=True)
        salario_reemplazo_otrosi = models.IntegerField(null=True, blank=True)
        tiquetes_otrosi = models.IntegerField(null=True, blank=True)
        matricula_otrosi = models.IntegerField(null=True, blank=True)
        seguro_medico_otrosi = models.IntegerField(null=True, blank=True)
        apoyo_bibliografico_otrosi = models.IntegerField(null=True, blank=True)
        costo_total_programa = models.IntegerField()
        ESTADO = (
                ('A', 'Con incumplimiento'),
                ('B', 'En curso'),
	...
	estado_agosto_2016= models.CharField(max_length=1, choices=ESTADO)
        titulo_otorgado = models.CharField(max_length=50, null=True, blank=True)
        fecha_titulacion = models.DateField(null=True, blank=True)
        observaciones = models.CharField(max_length=255)
	responsable_cierre = models.CharField(max_length=30, null=True, blank=True)
        soporte_base = models.FileField('/opt/si/comisiones/', blank=True)
        soportes_finales = models.FileField('/opt/si/comisiones/', blank=True)

class comisionv2Admin(admin.ModelAdmin):
	list_display = ('cc', 'nombres', 'apellidos', 'genero', 'fecha_nac', 'edad', 'facultad', 'proy_curricular', 'cat_docente', 'gr_inves_vinc', 'clas_colciencia', 'fecha_vinc', 'regimen_docente', 'tipo_estudio', 'tipo_apoyo_form_pos', 'modalidad_comision', 'prog_carg_adelanta', 'area_conocimiento_OCDE', 'entidad', 'pais', 'ciudad', 'becado', 'fecha_inicio_estudios', 'regimen_comision', 'duracion_estudios_meses', 'csu_ca_res', 'fecha_res_CSU_CA', 'num_res_aprobacion_CSU', 'num_contrato', 'fecha_contrato', 'duracion_contrato_meses', 'obj_contrato', 'num_pagare_poliza', 'remunerada', 'salario_RRHH', 'salario_reemplazo', 'tiquetes', 'matricula', 'seguro_medico', 'apoyo_bibliografico', 'gastos_instalacion', 'costo_inicial', 'otrosi',  'fecha_otrosi', 'obj_otrosi', 'valor_otrosi', 'salario_RRHH_otrosi', 'salario_reemplazo_otrosi', 'tiquetes_otrosi', 'matricula_otrosi', 'seguro_medico_otrosi', 'apoyo_bibliografico_otrosi', 'costo_total_programa', 'estado_agosto_2016', 'titulo_otorgado', 'fecha_titulacion', 'observaciones', 'responsable_cierre', 'soporte_base', 'soportes_finales')
